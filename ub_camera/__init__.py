...
class CameraGazebo(Camera):
	...
	def callback_Image(self, msg):
		...
		try:
			...
			if msg.pixel_format_type not in self._unsupported_pixel_formats:
				...
				self.logger.log(f'Error in gazebo image callback: {e}', severity=ub_utils.SEVERITY_ERROR)
		except Exception as e:
			self.logger.log(f'Error in gazebo image callback: {e}', severity=ub_utils.SEVERITY_ERROR)
	...
	def _changeFramerate(self, req_framerate):
		try:
			...
			return (False, 'cannot change Gazebo framerate from ub_camera; update the Gazebo sensor or world instead')
		except Exception as e:
	...
	def _changeResolution(self, req_height, req_width):
		try:
			if ((self.res_cols, self.res_rows) != (req_width, req_height)):
				return (False, 'cannot change Gazebo resolution from ub_camera; update the Gazebo sensor or world instead')
			else:
	...
	def stop(self, stopStream=True):
		...
		try:
			...
			self.logger.log(f'Could not stop cameraGazebo: {e}', severity=ub_utils.SEVERITY_ERROR)
...
class _WebSocketReceiveServer:
	...
	async def _handler(self, websocket):
		...
		self._cam.logger.log('CameraWebSocket: frame-pusher connected', severity=ub_utils.SEVERITY_INFO)
		try:
			async for message in websocket:
				if isinstance(message, bytes):
					self._cam.pushFrame(message)
		except Exception as e:
			self._cam.logger.log(f'CameraWebSocket: pusher error: {e}', severity=ub_utils.SEVERITY_WARNING)
		finally:
			self._pusher = None
			self._cam.logger.log('CameraWebSocket: frame-pusher disconnected', severity=ub_utils.SEVERITY_INFO)
...
class CameraWebSocket(Camera):
	...
	def _thread_receive(self, port):
		...
		try:
			...
			self.wsPort = 0
			self.camOn  = False
			self.logger.log('CameraWebSocket: receive server stopped', severity=ub_utils.SEVERITY_INFO)
	...
	def start(self, assetID=None, startStream=False, port=None,
		...
		if recv_port is None:
			self.logger.log('CameraWebSocket.start: no port specified', severity=ub_utils.SEVERITY_ERROR)
			return
	...
	def shutdown(self):
		...
		self.stopStream()
		self.camOn = False
		self.logger.log('CameraWebSocket: shutdown complete', severity=ub_utils.SEVERITY_INFO)
